...
public final class LogParserUtils {
    ...
    public static boolean skipParsingRule(final String parsingRule) {
        boolean skip = false;
        if (parsingRule == null || parsingRule.equals("") || parsingRule.charAt(0) == '#'
                || parsingRule.startsWith("\\s") || parsingRule.startsWith("\r") || // Carriage
                // return
...
//        }
    ...
    public static CompiledPatterns compilePatterns(final String[] parsingRulesArray,
                                                   final Logger logger) {
        ...
        for (int i = 0; i < parsingRulesArray.length; i++) {
            ...
            if (!skipParsingRule(parsingRule)) {
                try {
                    ...
                    // it
                    final String errorMsg = "Bad parsing rule:" + parsingRule + ", Error:"
                            + e.getMessage();
